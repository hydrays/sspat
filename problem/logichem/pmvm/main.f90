program ssa
  ...
  real(kind=8) max_SCnum, average_max_SCnum, average_wait_time 
...
!!$        end if
        ...
        ! if ( t > 2001.0 .and. x(4)+x(5).eq.0 ) then
...
!     xbar = xbar + x
     ...
     average_wait_time = average_wait_time + t
     !write (*, '(F10.2, 10F8.2)'), t, x, sum(x)
  end do
  average_max_SCnum = average_max_SCnum/real(NSample)
  average_wait_time = average_wait_time/real(NSample)

  write (*, '(10F12.2)'), pm, vm, takeover_counter, nottakeover_counter, &
       coexist_counter, average_max_SCnum, average_wait_time 
  end do 
  end do
end program ssa
