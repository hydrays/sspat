...
data7 <- read.csv('data/8dayslow.csv')
mcell <- as.numeric(data7[8,2:(ncol(data7[1,])-3)])
mcell[is.na(mcell)] <- 0
...
i <- 1
for ( r2 in seq(1, 2, by=0.02) ){
    j <- 1
      for ( d2 in seq(0.5, 2, by=0.02) ){
              for (k in seq(Nsample)){
                y <- Csimulator1(T, 0, r2, 0, d2, 0, 0)
                x[k] <- sum(y)
		}
              #dis <- cvmts.test(mcell, x)
